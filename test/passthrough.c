...
static bool lo_debug(fuse_req_t req) { return lo_data(req)->debug != 0; }
...
static void lo_do_readdir(fuse_req_t req, fuse_ino_t ino, size_t size,
	...
	while (1) {
		...
		if (plus) {
			...
			entsize = fuse_add_direntry_plus(req, p, rem, d->entry->d_name, &e,
											 nextoff);
		} else {
			struct stat st = {
			...
			entsize =
				fuse_add_direntry(req, p, rem, d->entry->d_name, &st, nextoff);
		}
		if (entsize > rem)
			break;
